...
Therefore, instead of using such a learning problem, we model the problem and
...
\begin{drools}[label=lst:instcosts,caption={[Paying for institution costs.]Paying for institution costs.}]
...
We can express this as whether the institutional account, which pays for facility costs, is positive at a time-step $t$. This function $\mathcal{S}(i,t)$ returns a boolean of whether the institution $i$ has endured until time $t$:
\begin{equation*}
...
deviation of all agents' accrued rewards. 
...
measuring, and all agents behaviour in a way which aims to lead to
...
Finally, the forth configuration introduces greedy agents into the conditions
from scenario 2. This will test firstly, whether greedy agents will negatively
...
\autoref{fig:powerbar} how individual rewards change for each group of greedy agents, when compared to the baseline.
