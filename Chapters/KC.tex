...
and knowledge, a so-called \emph{Knowledge Commons}. We then argue that
participatory sensing can be seen as such a common-pool resource system, and
therefore we can utilise Ostrom's theories for sustainable resource
management. Finally, we review governance in participatory sensing
...
\item \emph{analysts}, who process the data to generate conclusions and knowledge.
\end{enumerate}
...
enabled by advances in large-scale data processing, such that value knowledge
can be extracted from large datasets. This derived knowledge is the
...
described as a `Knowledge Commons', and that managing these commons as a common-pool
resource is key to their sustainability. In this section we address how
participatory sensing can be seen as such an information and knowledge commons.  We
give some background on the commons and introduce Ostrom's work on mangaging
...
commons, and therefore managed in the same way. % TODO: one more sentence?
...
can be defined by the application as action control. However, when dealing
...
from shared resource ownership coupled with incentive to free-riding could be
solved with appropriate governance.
