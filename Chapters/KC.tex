...
%From these deductions we can view participatory sensing as an information and knowledge commons. Therefore we can utilise the tools of this field in order to achieve sustainable management of the resource.
...
\citeasnoun{Pitt2012b} demonstrated that each of Ostrom's principles could be axiomatised for a resource-allocation problem. There are several differences between this problem and our provision and appropriation system. 
...
\>$\B{pow}(G, \m{apply}(A,I,\Role))=\true \holdsAt T$ \\
$\B{pow}(G, \m{apply}(A,I,\Role))=\true \holdsAt T \pad \leftarrow$ \\[0mm]
...
it is empowered to perform $\m{assign}$ action. This is the case if agent $A$ has applied for the role, agent $G$ occupies
...
Using roles we can define who will be \emph{empowered} to provision and appropriate certain resources, according to Figure~\ref{fig:psysex}.
...
As the nature of resource usage in this case is bursty (large number of requests in one go when route planning, then very few for hours afterwards), a quota system of $x$ requests per minute is extremely more restrictive than $60 \times 24 \times x$ requests per day, despite corresponding to the same overall quota.
...
Many such frameworks exist for human organisations, and some have been formalised to be usable for agent systems~\citep{Artikis2005}, including robust voting mechanisms~\citep{Pitt2005a}. 
...
\>$\B{pow}(H, \m{report}(M,A,I,O))=\true \holdsAt T$ \\
...
\> $\m{score\_threshold}(I)=T \holdsAt T \pad \land$ \\
\> $S<T$
\end{tabbing}
}}
...
\>$\m{derivedFrom}(\m{Source},\m{Alg},\m{Obj})=\true \pad \land$ \\
...
%As we said about equity as an outcome, we can compare a participant's received benefits with their efforts put into the system to achieve a measure of equity. 
...
\citeasnoun{Demirbas2010} leverage Twitter as infrastructure for information provision.
...
Thus, with the presentation of this framework we are taking the first steps to the supply of institutional governance for managing information and knowledge commons in participatory sensing applications.
