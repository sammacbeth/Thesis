...
\citet{Tampitsikas2011} execute electronic institutions using an extension 
...
%There is also an extension, the Object Event Calculus, which has been used for EI
...
specification, however no quantitative comparison is provided. % + SWRL
...
In this section we present a method of rewriting \ac{EC} axioms in a
...
\subsection{Translation of EC Electronic Institution Specifications}\label{sec:translate}
...
\begin{drools}[label=lst:ronrinitiates,caption={[Example of translated initiates clauses.]Example of translated initiates clauses. The original Prolog specification is shown in the comment above each rule.}]
...
\begin{drools}[label=lst:droolsholdsat,caption={[Example of a translated holdsAt clause.]Example of a translated holdsAt clause. The original Prolog specification is shown in the comment above the rule.}]
//	holdsAt( pow(B, second(B,M))=true, T ) :-
...
Returning to the \ac{RONR} example, we can see a case where this problem arises in the following clause:
...
\subsubsection*{Rule execution order}
...
Prolog implementation, and with 100 motions it is already more than half the
