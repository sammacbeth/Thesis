...
to incorporate this robust\footnote{Robust, in that there is little or no ambiguity over which actions can be taken, and who can perform them; and the protocol is not vulnerable to manipulation. This means that the protocol will be able to function effectively in most circumstances.}
...
specifying various protocols, include the a voting protocol from the aforementioned
...
This version was used in the original \ac{RONR} voting protocol implementation, and is shown in \autoref{lst:secpl}. To this we add a \texttt{doUpdate} implementation which simply asserts each
\texttt{happens} tuple.
...
\caption{Execution time of voting protocol using Cached Event Calculus with SWIProlog and
...
implementation in Java. We run another performance test using this Prolog
